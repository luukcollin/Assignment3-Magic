...
public class Sort {
    ...
    public List<Coin> sort(List<Coin> unsorted) {
        this.mergesort(unsorted, 0, unsorted.size());
//        List<Coin[]> sortedArray = Collections.synchronizedList(smallerArrays);
        List<Coin[]> sortedArray = smallerArrays;
        return new ArrayList<Coin>(Arrays.asList(sortedArray.get(smallerArrays.size() - 1)));
//        return Collections.synchronizedList(new ArrayList<Coin>(Arrays.asList(sortedArray.get(smallerArrays.size() - 1))));
     ...
     */
    public void mergesort(List<Coin> data, int lo, int hi) {
        if (hi - lo < 2)
            return;
        int mi = (lo + hi) / 2;
     ...
     * @param hi max index of the data array
     */
    public void merge(List<Coin> data, int lo, int mi, int hi) {
        // index wwi1 as a temp, i2 as the array data [lo, mi - 1] subscript, i3 as the array data [mi, hi - 1] subscript
